...
func (r *InvoiceRepository) Save(invoice *domain.Invoice) error {
	stmt, err := r.db.Prepare(
		`INSERT INTO invoices (id,account_id,amount,card_last_digits,description,payment_type,status,created_at,updated_at)
...
func (r *InvoiceRepository) FindByID(id string) (*domain.Invoice, error) {
	...
	err := r.db.QueryRow(
		` SELECT id,account_id,amount,card_last_digits,description,payment_type,status,created_at,updated_at 
		from invoices 
		where id = $1 `,
		id).Scan(
		&invoice.ID,
...
func (r *InvoiceRepository) UpdateStatus(invoice *domain.Invoice) error {
	...
	rows, err := r.db.Exec(`update invoices 
	                  set status = $1, 
					  updated_at = $2 
					  where id = $3
